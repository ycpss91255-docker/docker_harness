...
Grand total (all levels): **1055 tests**.

## Test docs by level / type

| Doc | Scope | Count |
|-----|-------|-------|
| [unit.md](unit.md) | `.claude/test/bats/unit/` -- one hook / script in isolation | 1047 |
...
`.sh` in `.claude/hooks/` and `.claude/scripts/` (42 hooks + 34 helper
