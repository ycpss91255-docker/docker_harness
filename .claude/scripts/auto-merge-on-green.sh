#!/usr/bin/env bash
# log-allow:script -- emits data-product output (Monitor protocol: per-transition snapshot + MERGED/FAIL terminal line); stdout IS the event stream consumed by the auto-merge-on-green skill's Monitor wrap.
#
...
#
# Division of labour (refs #211, ported from initialization#154/#155):
