#!/usr/bin/env bats
#
# log_spec.bats - Tests for vendored OTel-aligned log.sh (mirror of base@v0.37.0, refs #148).
...
setup() {
  ...
  LOG_SH="/work/.claude/scripts/lib/log.sh"
}
...
@test "_log_info text output has timestamp + aligned level + tag" {
  run --separate-stderr bash -c "
    ...
    source ${LOG_SH}; _log_info setup summary"
  assert_success
  assert_equal "${stderr}" ""
  [[ "${output}" =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2}T ]]
  [[ "${output}" == *"[setup] INFO : summary" ]]
}

@test "_log_err text output to stderr with timestamp" {
  run --separate-stderr bash -c "
    export LOG_FORMAT=text
    source ${LOG_SH}; _log_err build repo_failed"
  assert_success
  assert_equal "${output}" ""
  [[ "${stderr}" =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2}T ]]
  [[ "${stderr}" == *"[build] ERROR: repo_failed" ]]
}

@test "_log_warn text output uses WARN (not WARNING)" {
  run --separate-stderr bash -c "
    export LOG_FORMAT=text
    source ${LOG_SH}; _log_warn run repo_skipped"
  assert_success
  [[ "${stderr}" =~ 'WARN : repo_skipped'$ ]]
}
...
@test "_log_fatal text output to stderr" {
  run --separate-stderr bash -c "
    export LOG_FORMAT=text
    source ${LOG_SH}; _log_fatal init precondition_missing"
  assert_success
  [[ "${stderr}" =~ 'FATAL: precondition_missing'$ ]]
}
...
@test "text levels are right-aligned to 5 chars" {
  run --separate-stderr bash -c "
    export LOG_FORMAT=text
    source ${LOG_SH}
    _log_info  setup summary
    _log_debug setup dry_run_cmd
  "
  assert_success
  [[ "${lines[0]}" =~ 'INFO : summary'$ ]]
...
@test "text output joins multi-token message with spaces" {
  run --separate-stderr bash -c "
    export LOG_FORMAT=text
    source ${LOG_SH}; _log_err build repo_failed word2 word3"
  assert_success
  [[ "${stderr}" =~ 'ERROR: repo_failed word2 word3'$ ]]
}

@test "text output skips attr=val args in message" {
  run bash -c "
    export LOG_FORMAT=text
    source ${LOG_SH}; _log_info setup summary ws_path=/tmp conf_hash=abc"
  assert_success
  [[ "${output}" =~ 'INFO : summary'$ ]]
...
@test "text output uses display= attribute over body when present" {
  run bash -c "
    export LOG_FORMAT=text
    source ${LOG_SH}; _log_info setup summary 'display=Env regenerated OK' ws_path=/tmp"
  assert_success
  [[ "${output}" =~ 'INFO : Env regenerated OK'$ ]]
  refute_line --partial "summary"
  refute_line --partial "ws_path"
}

@test "JSON includes display= as attribute alongside registered body" {
  run bash -c "source ${LOG_SH}; _log_info setup summary 'display=Env regenerated OK'"
  assert_success
  [[ "${output}" == *'"body":"summary"'* ]]
...
@test "JSON timestamp is UTC with microsecond precision" {
  run bash -c "source ${LOG_SH}; _log_info setup summary"
  assert_success
  local ts
...
@test "_log_info and _log_debug route to stdout" {
  run --separate-stderr bash -c "
    export LOG_FORMAT=text
    source ${LOG_SH}; _log_info setup summary; _log_debug setup dry_run_cmd"
  assert_success
  assert_equal "${stderr}" ""
  [[ "${output}" == *"summary"* ]]
  [[ "${output}" == *"dry_run_cmd"* ]]
...
@test "_log_warn _log_err _log_fatal route to stderr" {
  run --separate-stderr bash -c "
    export LOG_FORMAT=text
    source ${LOG_SH}
    _log_warn  setup lint_fail
    _log_err   setup drift_detected
    _log_fatal setup unrecognised_arg
  "
  assert_success
  assert_equal "${output}" ""
  [[ "${stderr}" == *"WARN"* ]]
...
@test "non-TTY stdout emits JSON by default (auto-detect)" {
  run bash -c "source ${LOG_SH}; _log_info setup summary ws_path=/tmp"
  assert_success
...
@test "non-TTY stderr emits JSON for _log_err" {
  run --separate-stderr bash -c "source ${LOG_SH}; _log_err build repo_failed"
  assert_success
  assert_equal "${output}" ""
  [[ "${stderr}" == *'"severity_text":"ERROR"'* ]]
  [[ "${stderr}" == *'"body":"repo_failed"'* ]]
}
...
@test "LOG_FORMAT=auto is equivalent to unset (non-TTY -> JSON)" {
  ...
  out_auto="$(bash -c "export LOG_FORMAT=auto; source ${LOG_SH}; _log_info setup summary" 2>/dev/null)"
  out_unset="$(bash -c "unset LOG_FORMAT; source ${LOG_SH}; _log_info setup summary" 2>/dev/null)"
...
@test "JSON output contains custom attributes" {
  run bash -c "source ${LOG_SH}; _log_info setup summary ws_path=/tmp conf_hash=abc123"
  assert_success
  [[ "${output}" == *'"ws_path":"/tmp"'* ]]
...
@test "JSON severity_number: DEBUG=5 INFO=9 WARN=13 ERROR=17 FATAL=21" {
  ...
  run bash -c "
    ...
    _log_warn  setup lint_fail
    _log_err   setup drift_detected
    _log_fatal init  precondition_missing
  " 2>/dev/null
  assert_success
...
@test "JSON output is valid per-line (starts with { ends with })" {
  run bash -c "
    source ${LOG_SH}
    _log_info setup summary
    _log_info setup drift_detected
  "
  assert_success
  local count=0
  local line
...
@test "NO_COLOR=1 text omits ANSI" {
  ...
  assert_success
  [[ "${stderr}" != *$'\033['* ]]
  [[ "${stderr}" == *"ERROR: repo_failed"* ]]
}

# ── Event registry ─────────────────────────────────────────────────

@test "log-events.txt is loaded and contains summary" {
  run bash -c "source ${LOG_SH}; _log_is_registered summary && echo yes"
  assert_output "yes"
}
...
@test "log.lnav-format.json exists and contains format key" {
  local _f="/work/.claude/scripts/lib/log.lnav-format.json"
  [[ -f "${_f}" ]]
...
@test "log.lnav-format.json declares json: true" {
  run grep -q '"json": true' /work/.claude/scripts/lib/log.lnav-format.json
  assert_success
}
