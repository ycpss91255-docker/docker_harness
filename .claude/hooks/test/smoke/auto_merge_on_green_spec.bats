#!/usr/bin/env bats
# Tests for .claude/scripts/auto-merge-on-green.sh (issue #211; ported from
# ycpss91255/initialization#155 / #154).
#
# Strategy: PATH-stub `gh` so `gh pr view ...` prints a canned JSON fixture
# and every other `gh` subcommand (merge / update-branch) is a logged no-op.
# Drive the script with --no-arm --interval 0 --max-iterations N so the poll
# loop runs deterministically. The gh-call log asserts side effects (arm /
# update-branch).

load '../lib/test_helper'

setup() {
  SCRIPT="$(script auto-merge-on-green.sh)"
...
@test "arg validation: bad --merge-method -> exit 2" {
  run "${SCRIPT}" --repo o/r --pr 1 --merge-method octopus
  assert_failure 2
  assert_output --partial "--merge-method must be"
}

@test "MERGED -> exit 0 with MERGED line" {
  _fixture '{"state":"MERGED","mergeStateStatus":"CLEAN","statusCheckRollup":[{"name":"bats","status":"COMPLETED","conclusion":"SUCCESS"}]}'
...
@test "required check FAILURE while BLOCKED -> FAIL exit 1" {
  _fixture '{"state":"OPEN","mergeStateStatus":"BLOCKED","statusCheckRollup":[{"name":"bats","status":"COMPLETED","conclusion":"FAILURE"}]}'
  run "${SCRIPT}" --repo o/r --pr 1 --no-arm --interval 0 --max-iterations 1
  assert_failure 1
...
@test "BEHIND -> runs gh pr update-branch, stays pending" {
  _fixture '{"state":"OPEN","mergeStateStatus":"BEHIND","statusCheckRollup":[{"name":"bats","status":"COMPLETED","conclusion":"SUCCESS"}]}'
...
@test "pending check still running -> keeps polling (max-iter 124)" {
  _fixture '{"state":"OPEN","mergeStateStatus":"BLOCKED","statusCheckRollup":[{"name":"bats","status":"IN_PROGRESS","conclusion":null}]}'
