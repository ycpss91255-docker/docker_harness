...
## Local-CI gate before PR (refs #176 / #208)
...
Committing docs (CHANGELOG / TEST.md / `doc/**` / `*.md`) after the
green run does NOT re-trigger the gate — the hook allows a PR when only
documentation changed since the last marker. Repos with no detectable
CI mechanism fail-open (the gate does not block them). To skip the gate
deliberately (accepting the GH-CI round-trip risk), override for the
exact HEAD: `LOCAL_CI_ACK=<sha> gh pr create ...`.
