...
description: Arm GitHub-native auto-merge on a PR and watch it land via a single Monitor, handling strict-branch-protection staleness (BEHIND -> update-branch). Use right after gh pr create to land a single PR; composes the wait-pr-ci monitoring primitive.
---
...
the session ends -- this skill's job is to arm it, give you visibility, and
keep it unblocked under `strict` branch protection.
...
Monitor(
  ...
  command: ".claude/scripts/auto-merge-on-green.sh --repo <OWNER>/<REPO> --pr <n>",
...
2. Polls `gh pr view --json state,mergeStateStatus,statusCheckRollup` -- keyed
   ...
   GitHub enforces whatever checks branch protection requires -- here
...
Same contract as the `wait-pr-ci` siblings, so Monitor treats them uniformly.
...
- The `.github` org-profile repo is the known exception: a doc-only PR there
  leaves required checks permanently pending (paths filter), so auto-merge
  stalls -- merge it manually instead.

## See also

- `.claude/scripts/auto-merge-on-green.sh` -- the implementation (`--help`).
