...
└── .claude/                  # Claude Code 設定
    ...
    │   ├── auto_allow_rm_outside_git_tree.sh # 只做兩件事:證得出「每個 rm operand 解析成實體絕對路徑(展開變數、collapse ..、cd -P 解 symlink)後都不在任何 git working tree 內、也沒有藏 working tree(BFS,不穿 symlink,budget 全次呼叫共用)」時 allow(省掉 prompt);問得出理由時 ask 並指名 target 與 tree root。永遠不 deny — 真正的 guard 是 settings.json 的 ask Bash(rm:*),所以本 hook 沉默=交給人:parse 不了、jq 不在、crash、逾時、gate 沒醒全都落在同一個安全預設。解析不了的字(heredoc body、here-string、`bash -c --` 的 payload)只要含 rm token 一律不 allow;非 rm/cd 的 simple command 會清掉追蹤中的 cwd(`builtin cd`/`command cd`/eval 都涵蓋);assignment prefix 不餵自己那道指令;`$'...'`/`$"..."` 是 quoting 不是字面 `$`,指派 IFS 的指令一律不 allow(word splitting 是本 guard 的前提)。refs #290, ADR-00000015
